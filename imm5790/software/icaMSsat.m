function R = icaMSsat(X, fs, winLength, settlTime, overLap, filterType, order, fl, fh)
...
addpath('../') % Set path to icaMS.m here.
...
for i = 1:numWin
    ...
    % Apply ICAML:
    [S(:,win(settlTime*fs+1:end)) A(2*i-1:2*i,:)] = icaMS(Xt');
    ...
    if draw
        ...
        title 'Independent Source Signal 1', xlabel 'Time [s]', ylabel(['R = ',num2str(A(2*i-1,1)/A(2*i,1))])
        ...
        %subplot(3,2,6). stairs((1:numWin)*(winLength-overLap),RR(:,1)), xlabel 'Time [s]', ylabel 'R value', title(['\mu = ',num2str(mu),', \sigma = ',num2str(sigma)])
        drawnow
        pause
    end
end
